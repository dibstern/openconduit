...
describe("Integration: Session Switch History", () => {
	...
	it("switch_session broadcasts history_page with session messages", async () => {
		...
		// Wait for at least one delta (proves SSE pipeline works), then wait
		// for done. The done event is generated by the status poller which
		// independently consumes GET /session/status queue entries — give it
		// enough time to reach the busy→idle transition.
		await client.waitFor("delta", { timeout: 5_000 });
